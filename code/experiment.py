...
def run(data_path, learner_type, fraction):
    ...
    if fraction == 1.0:
        ...
        train_features, train_targets, test_features, actual = train_test_split(features, targets, fraction)

    if learner_type == 'decision_tree':
        learning_method = DecisionTree(attribute_names)
        learning_method.fit(train_features, train_targets)
        learning_method.visualize()
        predictions = learning_method.predict(test_features)

    elif learner_type == 'prior_probability':
        learning_method = PriorProbability()
    ...
    confusion_matrix1 = confusion_matrix(actual, predictions)
    accuracy1 = accuracy(actual, predictions)
    precision, recall = precision_and_recall(actual, predictions)
    f1_measure1 = f1_measure(actual, predictions)


    return confusion_matrix1, accuracy1, precision, recall, f1_measure1
